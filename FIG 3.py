...
def logprior(theta):
    ...
    if not (-50 <= Rven_5AU <= 50 and 0 <= Rb <= 50):
        return -np.inf
...
g.plot_2d(gdsamples, 'Rven', 'Rb', filled=True, contour_colors=['#1f77b4'])
...
print(f"Rven/(1 AU)^2: {rven_5au:.2f} +{rven_hi:.2f} -{rven_lo:.2f} (cpd/100t)")
