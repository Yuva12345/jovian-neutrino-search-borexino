...
def logprior(theta):
    ...
    if not (-10 <= Rsat_10AU <= 10 and 0 <= Rb <= 50):
        return -np.inf
...
g.plot_2d(gdsamples, 'Rsat', 'Rb', filled=True, contour_colors=['#1f77b4'])
...
print(f"Rven/(1 AU)^2: {rsat_10au:.2f} +{rsat_hi:.2f} -{rsat_lo:.2f} (cpd/100t)")
