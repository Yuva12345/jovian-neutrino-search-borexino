...
def loglikelihood(theta, data, sigma, dsun, dven):
    Rsun_1AU, Rven_5AU, Rb = theta
    model = Rsun_1AU * (1.0/dsun.value)**2 + Rven_5AU * (1.0/dven.value)**2 + Rb
    return -0.5 * np.sum(((model - data) / sigma) ** 2)

def logprior(theta):
    Rsun_1AU, Rven_5AU, Rb = theta
    
    if not (-50 <= Rven_5AU <= 50 and 0 <= Rb <= 50 and 0 <= Rsun_1AU <= 50):
        return -np.inf
    return 0
...
Rven_5AU_init = np.random.uniform(-50, 50, Nens)
Rb_init = np.random.uniform(0, 50, Nens)
inisamples = np.array([Rsun_1AU_init, Rven_5AU_init, Rb_init]).T

argslist = (data, sigma, dsun, dven)
...
labels = [
    ...
    r'\frac{\mathcal{R}_{\mathrm{ven}}}{(1\,\mathrm{AU})^2}\,(\mathrm{cpd}/100\mathrm{t})',
...
blue = '#1f77b4' 
g.triangle_plot([gdsamples], ['Rsun','Rven', 'Rb'], filled=True, contour_colors=[blue],line_args=[{'ls': '-', 'color': blue}],markers=None)
fig = plt.gcf()
ax = plt.gca()
...
rven_5au, rven_hi, rven_lo = median_and_cred(samples_emcee[:, 1])
rb, rb_hi, rb_lo = median_and_cred(samples_emcee[:, 2])

print(f"Rsun/(1 AU)^2: {rsun_1au:.2f} +{rsun_hi:.2f} -{rsun_lo:.2f} (cpd/100t)")
print(f"Rven/(5 AU)^2: {rven_5au:.2f} +{rven_hi:.2f} -{rven_lo:.2f} (cpd/100t)")
