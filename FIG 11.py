...
dsun1, dven1 = distance_array(time1) 
dsun2, dven2 = distance_array(time2)  

# --- Bayesian Model---
def loglikelihood(theta, data, sigma, dsun, dven):
    Rsun_1AU, Rven_1AU = theta
...
def logposterior(theta, data, sigma, dsun, dven):
    lp = logprior(theta)
    if not np.isfinite(lp):
        return -np.inf
    return lp + loglikelihood(theta, data, sigma, dsun, dven)
...
Rven_1AU_init = np.random.uniform(-50, 50, Nens)
inisamples = np.array([Rsun_1AU_init, Rven_1AU_init]).T
...
argslist1 = (data1, sigma1, dsun1, dven1)
samples1 = get_samples(argslist1)
argslist2 = (data2,sigma2,dsun2,dven2)
...
gdsamples1 = MCSamples(samples=samples1[:, 1], names=['Rven'], labels=[r'$\mathcal{R}_{\mathrm{ven}}$'])
gdsamples2 = MCSamples(samples=samples2[:, 1], names=['Rven'], labels=[r'$\mathcal{R}_{\mathrm{vem}}$'])

density1 = gdsamples1.get1DDensity('Rven')
density2 = gdsamples2.get1DDensity('Rven')
...
plt.xlabel(r'$\mathbf{\frac{\mathcal{R}_{\mathrm{ven}}}{(1\,\mathrm{AU})^2}}$ (cpd/100t)', fontsize=16)
...
def print_res(samples_emcee):
    ...
    rven_1au, rven_hi, rven_lo = median_and_cred(samples_emcee[:, 1])
    print(f"Rsun/(1 AU)^2: {rsun_1au:.2f} +{rsun_hi:.2f} -{rsun_lo:.2f} (cpd/100t)")
    print(f"Rven/(1 AU)^2: {rven_1au:.2f} +{rven_hi:.2f} -{rven_lo:.2f} (cpd/100t)")
