...
dsun1, dmar1 = distance_array(time1) 
dsun2, dmar2 = distance_array(time2)  

# --- Bayesian Model---
def loglikelihood(theta, data, sigma, dsun, dmar):
    Rsun_1AU, Rmar_2AU = theta
    integral1 = np.mean((1.0/dsun.value)**2)
    integral2 = np.mean((2.0/dmar.value)**2)
    model = Rsun_1AU * ((1.0/dsun.value)**2 - integral1)+ Rmar_2AU *( (2.0/dmar.value)**2 - integral2)
    return -0.5 * np.sum(((model - data) / sigma) ** 2)

def logprior(theta):
    Rsun_1AU, Rmar_2AU = theta
    
    if not (-13 <= Rmar_2AU <= 13):
        return -np.inf
...
Rmar_2AU_init = np.random.uniform(-13, 13, Nens)
inisamples = np.array([Rsun_1AU_init, Rmar_2AU_init]).T
...
argslist1 = (data1, sigma1, dsun1, dmar1)
samples1 = get_samples(argslist1)
argslist2 = (data2,sigma2,dsun2,dmar2)
...
gdsamples1 = MCSamples(samples=samples1[:, 1], names=['Rmar'], labels=[r'$\mathcal{R}_{\mathrm{mar}}$'])
gdsamples2 = MCSamples(samples=samples2[:, 1], names=['Rmar'], labels=[r'$\mathcal{R}_{\mathrm{mar}}$'])

density1 = gdsamples1.get1DDensity('Rmar')
density2 = gdsamples2.get1DDensity('Rmar')
...
plt.xlabel(r'$\mathbf{\frac{\mathcal{R}_{\mathrm{mar}}}{(2\,\mathrm{AU})^2}}$ (cpd/100t)', fontsize=16)
...
def print_res(samples_emcee):
    ...
    rmar_2au, rmar_hi, rmar_lo = median_and_cred(samples_emcee[:, 1])
    print(f"Rsun/(1 AU)^2: {rsun_1au:.2f} +{rsun_hi:.2f} -{rsun_lo:.2f} (cpd/100t)")
    print(f"Rmar/(2 AU)^2: {rmar_2au:.2f} +{rmar_hi:.2f} -{rmar_lo:.2f} (cpd/100t)")
