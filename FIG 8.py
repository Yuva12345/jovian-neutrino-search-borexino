...
def logprior(theta):
    ...
    if not (-10 <= Rsat_10AU <= 10 and 0 <= Rb <= 50 and 0 <= Rsun_1AU <= 50):
        return -np.inf
    return 0
...
inisamples = np.array([Rsun_1AU_init, Rsat_10AU_init, Rb_init]).T

argslist = (data, sigma, dsun, dsat)
...
labels = [
    ...
    r'\frac{\mathcal{R}_{\mathrm{sat}}}{(10\,\mathrm{AU})^2}\,(\mathrm{cpd}/100\mathrm{t})',
...
blue = '#1f77b4'
g.triangle_plot([gdsamples], ['Rsun','Rsat', 'Rb'], filled=True, contour_colors=[blue],line_args=[{'ls': '-', 'color': blue}],markers=None)
fig = plt.gcf()
ax = plt.gca()
...
print(f"Rjup/(5 AU)^2: {rsat_10au:.2f} +{rsat_hi:.2f} -{rsat_lo:.2f} (cpd/100t)")
