...
def loglikelihood_sun_jupiter(theta, data, sigma, dsun, djup):
    Rsun_1AU, Rjup_5AU = theta
    integral1 = np.mean((1.0/dsun.value)**2)
    integral2 = np.mean((5.0/djup.value)**2)
    model = Rsun_1AU * ((1.0/dsun.value)**2 - integral1) + Rjup_5AU * ((5.0/djup.value)**2 - integral2)    
...
inisamples_sun_jup = np.array([Rsun_1AU_init, Rjup_init]).T

argslist1 = (data, sigma, dsun, djup)
sampler_sun_jup = emcee.EnsembleSampler(Nens, 2, logposterior_sun_jupiter, args=argslist1)
...
def loglikelihood_sun_venus(theta, data, sigma, dsun, dven):
    Rsun, Rven = theta
    integral1 = np.mean((1.0/dsun.value)**2)
    integral2 = np.mean((1.0/dven.value)**2)
    model = Rsun * ((1.0/dsun.value)**2 - integral1) + Rven * ((1.0/dven.value)**2 - integral2)
    return -0.5 * np.sum(((model - data) / sigma) ** 2)
...
inisamples_sun_ven = np.array([Rsun_1AU_init, Rven_init]).T

argslist2 = (data, sigma, dsun, dven)
sampler_sun_venus = emcee.EnsembleSampler(Nens, 2, logposterior_sun_venus, args=argslist2)
...
def logposterior_sun_saturn(theta, data, sigma, dsun, dsat):
    lp = logprior_sun_saturn(theta)
    if not np.isfinite(lp):
        return -np.inf
...
inisamples_sun_sat = np.array([Rsun_1AU_init, Rsat_init]).T

argslist1 = (data, sigma, dsun, dsat)
sampler_sun_sat = emcee.EnsembleSampler(Nens, 2, logposterior_sun_saturn, args=argslist1)
...
def loglikelihood_sun_only(theta, data, sigma, dsun):
    Rsun_1AU = theta
    integral1 = np.mean((1.0/dsun.value)**2)
    model = Rsun_1AU * ((1.0/dsun.value)**2 - integral1)
    return -0.5 * np.sum(((model - data) / sigma) ** 2)
...
inisamples_sun_only = np.array([Rsun_1AU_init]).T

argslist3 = (data, sigma, dsun)
sampler_sun_only = emcee.EnsembleSampler(Nens, 1, logposterior_sun_only, args=argslist3)
...
def model_sun_jup(time, Rsun_1AU, Rjup_5AU, dsun, djup):
    integral1 = np.mean((1.0/dsun.value)**2)
    integral2 = np.mean((5.0/djup.value)**2)
    model = Rsun_1AU * ((1.0/dsun.value)**2 - integral1) + Rjup_5AU * ((5.0/djup.value)**2 - integral2)
    return model
    
def model_sun_ven(time, Rsun_1AU, Rven_1AU, dsun, dven):
    integral1 = np.mean((1.0/dsun.value)**2)
    integral2 = np.mean((1.0/dven.value)**2)
    model = Rsun_1AU * ((1.0/dsun.value)**2 - integral1) + Rven_1AU * ((1.0/dven.value)**2 - integral2)
    return model

def model_sun_sat(time, Rsun_1AU, Rsat_10AU, dsun, dsat):
    integral1 = np.mean((1.0/dsun.value)**2)
    integral2 = np.mean((10.0/dsat.value)**2)
    model = Rsun_1AU * ((1.0/dsun.value)**2 - integral1) + Rsat_10AU * ((10.0/dsat.value)**2 - integral2)
    return model

def model_sun_only(time, Rsun_1AU, dsun):
    integral1 = np.mean((1.0/dsun.value)**2)
    model = Rsun_1AU * ((1.0/dsun.value)**2 - integral1)
    return model
...
model_best_fit_jup = model_sun_jup(time_fine, rsun_sj, rjup_sj, dsun_fine, djup_fine)
model_best_fit_ven = model_sun_ven(time_fine, rsun_sv, rven_sv, dsun_fine, dven_fine)
model_best_fit_sat = model_sun_sat(time_fine, rsun_ss, rsat_ss, dsun_fine, dsat_fine)
model_sun_only_vals = model_sun_only(time_fine, rsun_so, dsun_fine)
...
plt.errorbar(time_shifted, data, yerr=sigma, fmt='o', color='red', markersize=6, alpha=0.7, label='Modulation data')

plt.plot(time_fine_shifted, model_best_fit_jup, 'k-', linewidth=2.5, 
         label=r'$\frac{\mathcal{R}_\mathrm{sun}}{(1~\mathrm{AU})^2} + \frac{\mathcal{R}_\mathrm{jup}}{(5~\mathrm{AU})^2}$')

plt.plot(time_fine_shifted, model_best_fit_ven, 'g-', linewidth=2.5, 
         label=r'$\frac{\mathcal{R}_\mathrm{sun}}{(1~\mathrm{AU})^2} + \frac{\mathcal{R}_\mathrm{ven}}{(1~\mathrm{AU})^2}$')

plt.plot(time_fine_shifted, model_best_fit_sat, color='magenta', linestyle='--', linewidth=2.5, 
         label=r'$\frac{\mathcal{R}_\mathrm{sun}}{(1~\mathrm{AU})^2} + \frac{\mathcal{R}_\mathrm{sat}}{(10~\mathrm{AU})^2}$')

plt.plot(time_fine_shifted, model_sun_only_vals, 'b--', linewidth=2.5, 
         label=r'$\frac{\mathcal{R}_\mathrm{sun}}{(1~\mathrm{AU})^2}$')


plt.xlabel(r'Days since 2019-10-01', fontsize=16)

plt.ylabel(r'$\delta\mathcal{R}\ \mathrm{[cpd/100t]}$', fontsize=16)
...
model_sj_data= model_sun_jup(time, rsun_sj, rjup_sj, dsun, djup)
model_sv_data= model_sun_ven(time, rsun_sv, rven_sv, dsun, dven)
model_ss_data= model_sun_sat(time, rsun_ss, rsat_ss, dsun, dsat)
model_so_data= model_sun_only(time, rsun_so, dsun)
