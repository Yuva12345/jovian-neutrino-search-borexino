...
dsun1, djup1 = distance_array(time1) 
dsun2, djup2 = distance_array(time2)  
...
inisamples = np.array([Rsun_1AU_init, Rjup_5AU_init]).T
...
argslist1 = (data1, sigma1, dsun1, djup1)
samples1 = get_samples(argslist1)
argslist2 = (data2,sigma2,dsun2,djup2)
...
gdsamples1 = MCSamples(samples=samples1[:, 1], names=['Rjup'], labels=[r'$\mathcal{R}_{\mathrm{jup}}$'])
gdsamples2 = MCSamples(samples=samples2[:, 1], names=['Rjup'], labels=[r'$\mathcal{R}_{\mathrm{jup}}$'])

density1 = gdsamples1.get1DDensity('Rjup')
density2 = gdsamples2.get1DDensity('Rjup')
...
plt.xlabel(r'$\mathbf{\frac{\mathcal{R}_{\mathrm{jup}}}{(5\,\mathrm{AU})^2}}$ (cpd/100t)', fontsize=16)
