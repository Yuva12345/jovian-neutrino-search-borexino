...
def logprior(theta):
    ...
    if not (0 <= Rjup_5AU <= 4 and 0 <= Rb <= 50 and 0 <= Rsun_1AU <= 50):
        return -np.inf
    return 0
...
blue = '#1f77b4' 
g.triangle_plot([gdsamples], ['Rsun','Rjup', 'Rb'], filled=True, contour_colors=[blue],line_args=[{'ls': '-', 'color': blue}],markers=None)
fig = plt.gcf()
ax = plt.gca()
